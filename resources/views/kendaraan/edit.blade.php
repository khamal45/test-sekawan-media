...
@section('content')
    <div class="max-w-2xl mx-auto p-4">
        <h1 class="text-2xl font-semibold mb-6">Edit Kendaraan</h1>
        ...
        <form action="{{ route('kendaraan.update', $kendaraan->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="jenis" id="jenis"
                    ...
                    <option value="angkutan orang"
                        {{ old('jenis', $kendaraan->jenis) == 'angkutan orang' ? 'selected' : '' }}>Angkutan Orang</option>
                    <option value="angkutan barang"
                        {{ old('jenis', $kendaraan->jenis) == 'angkutan barang' ? 'selected' : '' }}>Angkutan Barang
                    </option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="tipe" id="tipe"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400"
                    required>
                    <option value="" disabled {{ old('tipe', $kendaraan->tipe) == null ? 'selected' : '' }}>-- Pilih
                        Tipe --</option>
                    <option value="milik" {{ strtolower(old('tipe', $kendaraan->tipe)) == 'milik' ? 'selected' : '' }}>
                        Milik</option>
                    <option value="sewaan" {{ strtolower(old('tipe', $kendaraan->tipe)) == 'sewaan' ? 'selected' : '' }}>
                        Sewaan</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="region" id="region"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400"
                    value="{{ old('region', $kendaraan->region) }}" required>
            </div>
            ...
            <div>
                ...
                <input type="date" name="jadwal_service_berikutnya" id="jadwal_service_berikutnya"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400"
                    value="{{ old('jadwal_service_berikutnya', $kendaraan->jadwal_service_berikutnya ? \Carbon\Carbon::parse($kendaraan->jadwal_service_berikutnya)->format('Y-m-d') : '') }}"
                    required>
            </div>
